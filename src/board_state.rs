use crate::chess_move::{Move, MoveType, Position};
use crate::color::Color;
use crate::move_gen::{
    generate_bishop_moves_helper, generate_king_moves_helper, generate_knight_moves_helper,
    generate_rook_moves_helper,
};
...
impl BoardState {
    ...
    pub fn new(fen: &str) -> Result<BoardState, &str> {
        // Creating an 10 x 10 array of uninitialized arrays
        let blank_state = Square {
            piece: None,
            color: (Color::White),
        };

        // Setting to white and then updating later
        // Assigning colors, but not charged
        let mut squares = [[blank_state; 10]; 10];
        ...
        let active_color = match fen_strings.get(1) {
            Some(&"w") => Color::White,
            Some(&"b") => Color::Black,
    ...
    fn parse_fen_entry(entry: char) -> Result<Option<Piece>, String> {
        match entry {
            'r' => Ok(Some(Piece {
            ...
            _ => Err("Not a valid fen piece!".to_string()),
        }
    }
    ...
    pub fn make_move(&mut self, current_move: &Move) {
        ...
        match move_type {
            MoveType::Standard(val) => {
                ...
                //Setting enpassant
                if val.piece_moved.piece_type == PieceType::Pawn
                    && abs(val.after.x as i8 - val.before.x as i8) == 2
                {
            ...
            MoveType::Castle(val) => {
                let y_positions = if val.is_kingside {
                    vec![5, 8, 7, 6]
                } else {
                    vec![5, 1, 3, 4]
                };
                ...
                self.squares[8][y_positions[2]].piece = Some(Piece {
                    piece_type: PieceType::King,
                    color: self.active_color,
                });
                self.squares[8][y_positions[3]].piece = Some(Piece {
                    piece_type: PieceType::Rook,
                    color: self.active_color,
                });
            }
            ...
            MoveType::EnPassant(val) => {
                self.squares[val.before.x][val.before.y].piece = None;
                self.squares[val.after.x][val.after.y].piece = Some(Piece {
                    piece_type: PieceType::Pawn,
                    color: self.active_color,
                });
    ...
    */
    pub fn is_in_check(self: &BoardState) -> bool {
        ...
        match board.active_color {
            Color::White => {
                pawn_square_right = Position {
                    x: king_pos.x - 1,
                    y: king_pos.y - 1,
                };
                pawn_square_left = Position {
                    x: king_pos.x - 1,
                    y: king_pos.y + 1,
                };
            }
        ...
        if pawn_square_right.is_valid_position()
            && board.squares[pawn_square_right.x][pawn_square_right.y].is_occupied()
        {
            let p = board.squares[pawn_square_right.x][pawn_square_right.y]
                .piece
                .unwrap();
        ...
        if pawn_square_left.is_valid_position()
            && board.squares[pawn_square_left.x][pawn_square_left.y].is_occupied()
        {
            let p = board.squares[pawn_square_left.x][pawn_square_left.y]
                .piece
                .unwrap();
            if p.piece_type == PieceType::Pawn && p.color != board.active_color {
                println!("in check by pawn");
        ...
        candidates = generate_king_moves_helper(&king_pos);
